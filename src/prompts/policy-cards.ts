...
For covered-call, protective-put, hedge, income, catalyst-volatility, or options strategy prompts, keep the strategy tied to the option-chain underlying, the user's owned underlying or intended exposure, stated cost basis, share count, horizon, risk tolerance, and event catalyst when supplied. Use quote and option-chain evidence before naming strikes, expirations, premiums, Greeks, liquidity, or implied volatility, and state the tool-output date when those values are used. If the user has not supplied enough position context for a personalized trade, make the smallest useful assumption, label it, and frame the answer as a strategy read rather than pretending to know the full account. For covered calls, discuss premium received, assignment risk, capped upside, share-price downside that the premium does not protect, IV or earnings-event risk, exit liquidity, return-if-assigned, and why a stock is a good covered-call candidate only if the investor is willing to sell at the strike. Show assignment outcomes for below-strike, near-strike, and above-strike expirations; include ex-dividend or earnings assignment risk when relevant; compare premium to stock value as a simple and annualized premium yield; and explain when to roll, close, or let assignment happen. For a protective put:
- Size the hedge in whole contracts against the stated share count and verify the option's actual contract multiplier rather than assuming the common 100-unit default, because adjusted or non-standard contracts exist. State whether rounding down leaves residual uncovered shares that remain fully exposed or rounding up creates surplus long put exposure.
- State the strike sell right for the shares actually covered: the long put gives the right to sell those covered shares at the strike, so at expiration their protected value does not fall below the strike and the premium reduces net protection.
- Only when the contracts fully match the owned shares per covered share may you quote a single numerical per-share floor such as strike minus the premium paid; do not label that figure an aggregate floor for the whole position.
- When contract coverage differs from the owned shares, do not quote a single numerical per-owned-share floor. State instead the qualitative strike sell right for the covered shares, the actual total premium paid, and either residual uncovered shares or surplus long put exposure, accounting for the premium and payoff of any surplus puts separately.
- Distinguish the standalone long put, where the put leg itself can lose no more than the premium paid, from the combined stock-plus-put loss, which also includes the stock's decline down to the strike, the premium, any uncovered shares, and any surplus-put payoff.

Cover premium decay, bid/ask, open interest, liquidity, delta, theta, implied volatility, and what would make the hedge too expensive. Require a cost basis for total or net P/L, and allow a clearly labeled current-mark scenario when cost basis is unavailable. If the option chain is stale, empty, or unusable, disclose that data gap but do not stop at cannot rank contracts; Do not create a ranked contract table or invented Greeks, OI, IV, or bid/ask values. Instead use sections such as How it works, Strike and expiration tradeoff, labeled hypothetical example, whether it makes sense, Alternatives, and What live facts would change the answer. The labeled hypothetical should include stock price, strike, premium, the per-share hedge floor for covered shares, max loss before and after the hedge, and premium as a percent of the stock position, and must label covered versus uncovered shares instead of presenting the strike-minus-premium figure as the floor for the whole position. Disclose stale quotes, missing expirations, wide bid/ask spreads, missing Greeks, or unavailable event-volatility evidence instead of inventing precision. End with the strategy choice, why it fits or does not fit the stated objective, the main risk/downside, and the conditions that would invalidate the setup.`,
  },
  backtest_review: {
    id: "backtest_review",
