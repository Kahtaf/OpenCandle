...
- None of `acpx`, `@agentclientprotocol/codex-acp`, or `@agentclientprotocol/claude-agent-acp` are repo devDependencies. Each is resolved at call time, in order: an explicit `*_COMMAND` override below; then a global install found on PATH (an inherited `node_modules/.bin` entry is always stripped first, so a repo-local install can never shadow it); then `npx --yes <package>@<pinned range>` on demand, which resolves into npm's own npx cache rather than this repo's `node_modules`. This matters because `@agentclientprotocol/codex-acp` bundles its own pinned `@openai/codex` as a transitive dependency — see the `CODEX_PATH` note below for how the global Codex CLI is still guaranteed to run instead of that bundled copy.
- `OPENCANDLE_COMPETITIVE_ACPX_COMMAND`: optional acpx command override. Defaults to a global `acpx` on PATH, falling back to `npx --yes acpx@^0.13.2`.
- `OPENCANDLE_COMPETITIVE_CLAUDE_AGENT_COMMAND`: optional Claude ACP adapter override. Defaults to a global `claude-agent-acp` on PATH, falling back to `npx --yes @agentclientprotocol/claude-agent-acp@^0.70.0`. The runner also sets `CLAUDE_CODE_EXECUTABLE` to the global `claude` CLI (unless already set) so the adapter drives the operator's own logged-in Claude Code install.
- `OPENCANDLE_COMPETITIVE_CODEX_AGENT_COMMAND`: optional Codex ACP adapter override. Defaults to a global `codex-acp` on PATH, falling back to `npx --yes @agentclientprotocol/codex-acp@^1.7.0`. The runner also sets `CODEX_PATH` to the global `codex` CLI (unless already set): the adapter honors `CODEX_PATH` and otherwise silently falls back to whatever Codex version it bundles as a dependency, so this is required to make the baseline actually exercise the operator's own Codex install/auth instead of a stale bundled one.
...
- `OPENCANDLE_COMPETITIVE_REQUIRE_ALL`: set to `1` to fail when any baseline fails preflight. By default, unavailable local baselines are recorded under `skippedCompetitors` and the loop continues with the available agents. A skip is never silent: the recorded reason names what is missing (a CLI not on PATH, acpx/an adapter unreachable via npx) and how to fix it, for example "install/log in to Codex ... or set OPENCANDLE_COMPETITIVE_CODEX_AGENT_COMMAND".
...
`acpx` requires its ACP adapter binaries to be available on PATH or passed through `--agent`. None of `acpx`, `@agentclientprotocol/codex-acp`, or `@agentclientprotocol/claude-agent-acp` are repo dev dependencies — a repo-local install would risk `npm run`'s `node_modules/.bin`-first PATH silently shadowing the operator's own global, logged-in CLI with whichever version an adapter happens to bundle. Instead `npm run eval -- competitive` resolves each on demand (global install on PATH, else `npx --yes <package>@<pinned range>`) so it still gets the structured ACP path instead of raw CLI/PTTY scraping, without ever touching this repo's `node_modules`. The Gemini baseline does not require `acpx` when API mode is selected. If forced to ACP mode, it uses the local `gemini --acp --skip-trust` command with `GEMINI_CLI_TRUST_WORKSPACE=true`.
