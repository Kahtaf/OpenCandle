...
3. **Run the baseline (MANDATORY — policy decision 2026-07-03: credentialed e2e evals run; they are not skipped).**
   - Run `npm run eval:router-live` with the router-model credential present (`ANTHROPIC_API_KEY` or the configured provider key).
   - Save the full output to `tests/fixtures/router/eval-baselines/<date>.txt`.
   ...
   - If no credential is available in your environment: **STOP the whole package and report** — do not proceed to archival. Request a credentialed environment from the maintainer. There is no skip path; an archived baseline file is a hard prerequisite for WP2's archival step.
   - **Already on record:** `tests/fixtures/router/eval-baselines/2026-07-03-gemini-2.5-flash.txt` — a live run from the planning session against `gemini-2.5-flash` (the only credential available at the time). Result: **9/26 pass (34.6%), p50 1924ms / p95 2184ms** — far below the gate. This is NOT the canonical baseline (the gate is defined against the production default `claude-haiku-4-5`, and the fixtures were recorded with a Claude model; heavy slot-shape drift on non-Claude models is a known, partially-mitigated issue). It IS admissible evidence of cross-model contract drift and must be cited in the design.md addendum as such. The canonical run in this step therefore still requires `ANTHROPIC_API_KEY` with default provider/model (no `OPENCANDLE_ROUTER_PROVIDER`/`OPENCANDLE_ROUTER_MODEL` overrides). One concrete triage lead to record verbatim for the reviewer: fixture 022 (`cpi-as-metric`) routed to `workflow_dispatch`/`compare_assets` with `CPI` surviving in `slots.symbols` despite CPI being in the acronym-drop dictionary — potentially a real gap in slot-level acronym filtering on non-Claude models, not just shape drift.
