...
**Audience:** an implementation agent executing spec/document edits, archival, and — in WP2 and WP7 only — test/eval-tooling code. This plan involves **no production code changes** and **no feature implementation**: WP7 authorizes new *test* files to close verification gates, and WP2 authorizes one eval-script edit (Pi-auth model resolution in `tests/scripts/run-live-router-eval.ts`), nothing more. Outside those, if you find yourself editing anything under `src/`, `gui/`, or `tests/` (other than reading), you have left the scope of this plan. Feature implementation for the two changes that survive this cleanup (`forget-command`, `gui-session-scoped-action-cleanup`) is owned by `docs/internal/high-leverage-improvements-plan.md` items I4 and I7, whose completing PRs also archive those changes — so the backlog reaches zero active changes at the end of that plan, not this one.
...
3. **Run the baseline (MANDATORY — policy decision 2026-07-03: credentialed e2e evals run; they are not skipped).**

   **Auth decision (2026-07-03, maintainer-confirmed): no raw `ANTHROPIC_API_KEY` is assumed. Claude access goes through Pi's sign-in auth — the same path production routing uses.** Production resolves the router model from the live Pi session (`resolveRouterLlmClient` in `src/pi/opencandle-extension.ts` uses `ctx.model`, backed by Pi's auth storage), so an eval that resolves the model the same way is MORE faithful than the current API-key path, not less. acpx was considered and REJECTED for this eval: it drives a full agent CLI (own system prompt, tools, sampling), so it would not exercise the router's raw prompt→JSON `completeSimple` contract, and its latency/cost numbers would be meaningless against the gate. acpx remains correct only for the competitive benchmark's generic-agent baselines.

   - **Authorized eval-tooling edit (test code, in scope for this package):** extend `tests/scripts/run-live-router-eval.ts` with a Pi-auth resolution mode (e.g. `--auth pi` or automatic fallback when no API key is present) that resolves the model via the Pi model registry + auth storage from `~/.pi/agent/` — the same mechanism the GUI server's model-setup service and the harness's full-session runs already use. Keep the existing API-key path working. Timebox the attempt; if the pi-ai/pi-coding-agent APIs genuinely cannot support this from a standalone script, fall back per the next bullet and record why.
   - **Primary path:** run `npm run eval:router-live` via Pi auth against the fixture-recording model family (default `claude-haiku-4-5`). Save output to `tests/fixtures/router/eval-baselines/<date>.txt`; record pass-rate and p50/p95 in the design.md addendum. If pass-rate < 90%, triage is OUT of scope: record failures verbatim and flag the PR "ROUTER BASELINE BELOW GATE — needs triage". Do not edit fixtures to make them pass.
   - **Fallback path (only if Pi-auth resolution is infeasible):** the gate model becomes `gemini-2.5-flash`, whose live baseline is already archived (see below) and already satisfies the "real credentialed run on record" requirement — WP2 is not blocked either way. In that case the design.md addendum must state that the Claude-family baseline remains unmeasured and that improvements item I9 (Gemini router-contract hardening) owns closing the gap to the gate.
   - **Already on record:** `tests/fixtures/router/eval-baselines/2026-07-03-gemini-2.5-flash.txt` — a live run from the planning session against `gemini-2.5-flash` (the only credential available at the time). Result: **9/26 pass (34.6%), p50 1924ms / p95 2184ms** — far below the gate. This is NOT the Claude-family baseline (the fixtures were recorded with a Claude model; heavy slot-shape drift on non-Claude models is a known, partially-mitigated issue) — but note that production routing follows the USER'S selected Pi model, so for Gemini users this drift is live behavior today, which is why improvements item I9 exists. It IS admissible evidence and must be cited in the design.md addendum. The Claude-family run in this step uses the Pi-auth mode above (no API key). One concrete triage lead to record verbatim for the reviewer: fixture 022 (`cpi-as-metric`) routed to `workflow_dispatch`/`compare_assets` with `CPI` surviving in `slots.symbols` despite CPI being in the acronym-drop dictionary — potentially a real gap in slot-level acronym filtering on non-Claude models, not just shape drift.
...
5. Validate strictly, then archive **once the baseline from step 3 is recorded** — archival proceeds even if the score is below the gate (the promotion already shipped in 0.11.0; what matters is that the truth is on record and triage is escalated), but NEVER without a real credentialed run archived. If below gate, the PR description must carry the "ROUTER BASELINE BELOW GATE — needs triage" flag prominently.
