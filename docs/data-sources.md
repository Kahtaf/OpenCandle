...
| Fundamentals | `get_company_overview`, `get_financials`, `get_earnings`, `compute_dcf`, `compare_companies` | London Strategic Edge (when configured), then Alpha Vantage, then Yahoo Finance for financial statements and DCF (Yahoo Finance also backstops market cap); Alpha Vantage with Yahoo Finance fallbacks for overview, earnings, and comparisons |
