...
OpenCandle uses model providers for synthesis, but the finance workflow starts with explicit tools for quotes, options, filings, macro data, sentiment, fundamentals, crypto data, and portfolio context. A quote answer can show the provider and timestamp; a filing answer can point to SEC EDGAR results; an options answer can preserve per-share versus per-contract context; a macro answer can name the FRED series; and a portfolio answer can separate saved local holdings from new user input.
