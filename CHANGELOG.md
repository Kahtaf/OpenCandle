...
- Creating or editing an alert now opens on the rule it will save, written in the same words the saved rule row uses and restated live as the form changes, with its repeat window underneath in human units. Each condition option carries a one-line description of what it watches, a price threshold prefills from the symbol's current quote with a quiet note of how far the level sits from it as the level is typed, and switching to a condition that measures something else clears a level that no longer applies. The save action now matches the other market-state sheets. A new `SelectField` primitive, built on Radix Select, backs the described options.
...
- The GUI watchlist page is now a full-canvas workbench: a quote board that takes the available width with a sticky column header and its own scrolling, beside a fixed symbol rail. The trend column is chart only with no label or caption, the change column is one signed line with the percent inline, and an extended-session price renders as an amber pre-market or blue after-hours chip inside the price column instead of a sub-line drifting across columns. The row shown in the rail is marked as the current selection, the volume column steps aside in the width band where the rail claims the space it needs, and the table keeps stable column widths at every desktop size. In the rail, the ticker itself links to the symbol page, the primary action is Create alert, and removal is a quiet secondary that still confirms first.
