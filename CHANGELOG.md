...
- Shared hosted-runtime hardening now rejects malformed OHLC history without inventing intraday timestamps, preserves overall Pi stream deadlines in writer and follower tabs, prevents read-only follower bootstraps from triggering invalidation loops, keeps in-flight market mutations idempotent under cache pressure, supports named SQLite parameters in the WASM adapter, identifies SEC reachability probes correctly, and opens newer native SQLite schemas without applying write-affecting pragmas. Portfolio views now keep unknown quote currencies and unavailable valuations explicit while retaining known cost basis, and the relay returns bounded client errors for malformed header values while its live smoke runs in an inert same-origin document that cannot expose test credentials to application scripts.
