...
- Hosted follower tabs now derive writability from their own coordinator role even when the writer supplies their bootstrap, and ask responsive older writers to hand over the single browser runtime before validating or saving model and data-provider API keys. Hidden active tabs no longer block credential setup, follower controls remain read-only, and secrets stay confined to the requesting tab.
