...
- Shared hosted-runtime hardening now rejects malformed OHLC history without inventing intraday timestamps, waits for every coordinator-managed Pi workflow prompt before checkpointing, preserves overall Pi stream deadlines in writer and follower tabs, keeps the runtime alive across back-forward cache restores, prevents read-only follower bootstraps from triggering invalidation loops, keeps in-flight market mutations idempotent under cache pressure, cleans up failed process writes, preserves the session selected while another run completes, retries transient relay negotiation, separates model-key clearing from market-provider credentials, supports named SQLite parameters in the WASM adapter, identifies SEC reachability probes correctly, and opens newer native SQLite schemas without applying write-affecting pragmas. Hosted watchlists and alerts now use canonical instrument resolution and validate alert mutations before persistence; non-interactive Pi surfaces direct provider setup to the shared settings flow. Portfolio updates are scoped to the selected portfolio, while views keep unknown quote currencies, partially unavailable valuations, and aggregate P&L explicit while retaining known cost basis. The relay returns bounded client errors for malformed header values, while its live smoke runs in an inert same-origin document that cannot expose test credentials to application scripts.
