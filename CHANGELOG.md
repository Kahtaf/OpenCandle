...
- Hosted web now uses Pi's concrete `ModelRuntime`, provider implementations, model discovery, authentication, streaming, and completion paths for the full browser-safe OpenAI, Anthropic, and Google catalogs instead of an OpenAI-only compatibility client. The local GUI, TUI, and hosted web share provider metadata, model selection rules, routing behavior, attachment parsing, `ask_user` lifecycle, tool/workflow metadata, and canonical Pi session records; hosted credentials remain provider-scoped browser secrets.
...
- Shared hosted-runtime hardening now rejects malformed OHLC history without inventing intraday timestamps, waits for every coordinator-managed Pi workflow prompt before checkpointing, preserves overall Pi stream deadlines in writer and follower tabs, keeps the runtime alive across back-forward cache restores, prevents read-only follower bootstraps from triggering invalidation loops, keeps market mutations idempotent across cache pressure and persistence retries, checkpoints state after failed model turns, cleans up failed process writes, preserves the session selected while another run completes, revalidates relay capabilities, separates model-key clearing from market-provider credentials, bounds archive state before decoding, unwraps acknowledged GUI mutations, validates sentiment price evidence consistently, supports named SQLite parameters in the WASM adapter, identifies SEC reachability probes correctly, and opens newer native SQLite schemas without applying write-affecting pragmas. Hosted watchlists and alerts now use canonical instrument resolution, preserve omitted alert-update fields, accept zero cooldown, and validate mutations before persistence; non-interactive Pi surfaces direct provider setup to the shared settings flow. Portfolio updates are scoped to the selected portfolio, while views keep unknown quote currencies, incomplete valuations, and aggregate P&L explicit while retaining all known base-currency cost basis. The relay returns bounded client errors for malformed header values, while its live smoke runs in an inert same-origin document that cannot expose test credentials to application scripts.
