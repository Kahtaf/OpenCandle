...
- **Ticker-disambiguation dual-run slice** — the prompt-to-policy migration now selects `ticker_disambiguation` as the single V1 slice, injects its replacement policy in dual-run mode, and documents that legacy ticker clauses remain authoritative until parity passes.
