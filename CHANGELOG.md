...
- The live GUI/TUI parity eval now passes end to end and asserts truthful contracts: identical `opencandle-*` entry-type sets across both surfaces (excluding run-conditional provider-gap entries), session-addressed snapshot reads, and the projector's actual completed-run state. It surfaced a documented finding: the `/analyze` transform path emits no `opencandle-workflow` entry (only router-dispatch paths do), so dashboard analysis tracking never sees comprehensive analysis — fixing that requires an ask-first extension change and is recorded as follow-up work.
