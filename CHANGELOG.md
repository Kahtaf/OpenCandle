...
- Hosted Pi runtime follow-up hardening now authenticates originless private HTTP requests with an ephemeral runtime token, keeps the writer lock through checkpoint teardown, deduplicates matching in-flight runs without retaining full session archives, rejects reused action IDs with changed inputs, refreshes canonical state after failed or cancelled streams, restores corrupt offline state from backup, prevents cleared session keys from returning through a follower tab, and advertises only provider capabilities that the browser relay can actually execute.
