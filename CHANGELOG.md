...
- Creating or editing an alert now opens on the rule it will save, written in the same words the saved rule row uses and restated live as the form changes, with its repeat window underneath in human units. Each condition option carries a one-line description of what it watches, a price threshold prefills from the symbol's current quote with a quiet note of how far the level sits from it as the level is typed, a prefilled level leaves when the symbol it was read from is replaced so it cannot be saved against the next one, and switching to a condition that measures something else clears a level that no longer applies. The save action now matches the other market-state sheets. A new `SelectField` primitive, built on Radix Select, backs the described options.
