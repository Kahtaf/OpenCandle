...
- Hosted model-key setup now completes the top-level Turnstile authorization before starting the WebContainer probe and allows enough time for that first browser-only exchange, instead of leaving the stdio request pending behind the native five-second probe deadline.
