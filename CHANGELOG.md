...
- Hosted model-key setup now completes the top-level Turnstile authorization before starting the WebContainer probe, restarts a runtime that previously booted without relay configuration, and allows enough time for that first browser-only exchange, instead of leaving the stdio request pending behind the native five-second probe deadline.
