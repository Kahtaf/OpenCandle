...
- Hosted relay and persistence follow-up hardening now gates every Pi model request on the negotiated relay manifest, keeps model-stream inactivity limits active after response headers, cancels the producer when a follower abandons a stream, preserves unchanged SQLite state across session-only checkpoints, rejects implausible action timestamps and sparse chart payloads, prevents partial portfolio totals when a base-currency quote is unavailable, and represents credential requirements directly in hosted tool policies instead of a separate Alpha Vantage exception list.
