...
- **Prompt-policy review hardening** — manifest hard assertions now fail when unregistered, product evals use family-aware output-shape checks instead of broad keyword shortcuts, and scorecards flag regex-inferred structured diagnostics as observe-only until typed answer metadata exists.
...
- **Macro-allocation replacement-active slice** — macro portfolio review guidance now routes through the `macro_allocation_review` policy card and answer contract, with accepted owner-promotion parity preserving macro evidence use, provider-gap continuation, structural portfolio reads, actionable adjustments, and watchlist/invalidation.
- **Options-strategy replacement-active slice** — covered-call and protective-put guidance now routes through the `options_strategy` policy card and answer contract, with old-vs-current ref parity preserving options workflow dispatch, owned-underlying context, option-chain evidence, risk framing, freshness, gaps, and source coverage.
- **Portfolio-review replacement-active slice** — existing-allocation critique now routes through the `portfolio_review` policy card and answer contract, with old-vs-current ref parity preserving agent-task routing, no budget clarification, structural allocation review, risk/downside, data-gap disclosure, and source coverage.
- **Backtest-review replacement-active slice** — backtest metric reporting now routes through the `backtest_review` policy card and answer contract, with accepted owner-promotion parity preserving `backtest_strategy` tool use while moving metric coverage, practical-edge, data-gap, and source-coverage obligations out of generic single-asset planning.
- **Stateful-tracking replacement-active slice** — watchlist and prediction state turns now route through the `stateful_tracking_update` policy card and answer contract, with accepted owner-promotion parity preserving `watchlist_or_tracking` routing and `track_prediction` persistence behavior.
- **Product eval comparison hardening** — product eval scoring now recognizes commitment-led portfolio answers, 30-day/next-month options horizons, and incomplete/caution sentiment risk framing, reducing false negatives found during main-vs-branch comparison runs.
