...
const WORKFLOW_SCHEMAS = {
  ...
  comprehensive_analysis: {
    ...
    description: "Run market, fundamentals, technical, sentiment, and macro analysts on one ticker, then synthesize.",
    fields: [
      { name: "symbol", kind: "symbol", label: "Symbol", required: true, default: "NVDA" },
      { name: "depth", kind: "segmented", label: "Depth", default: "standard", options: [
  ...
  portfolio_builder: {
    ...
    description: "Build a draft portfolio from goals and constraints. The agent will fetch candidates, check correlation, and present an allocation table.",
    fields: [
      { name: "objective", kind: "freetext", label: "Objective", required: true,
        placeholder: "Long-term growth with US tech exposure", maxLength: 200,
        helper: "One or two sentences describing what the portfolio is for." },
      { name: "budget", kind: "money", label: "Budget", default: 100000, helper: "Total amount to allocate." },
      { name: "riskProfile", kind: "segmented", label: "Risk profile", default: "balanced", options: [
        { value: "conservative", label: "Conservative" },
        { value: "balanced", label: "Balanced" },
        { value: "aggressive", label: "Aggressive" },
      ] },
      { name: "timeHorizon", kind: "segmented", label: "Time horizon", default: "5y", options: [
        { value: "1y", label: "1Y" }, { value: "3y", label: "3Y" }, { value: "5y", label: "5Y" }, { value: "10y", label: "10Y+" },
      ] },
      { name: "assetScope", kind: "segmented", label: "Asset scope", default: "stocks", options: [
        { value: "stocks", label: "Stocks" }, { value: "stocks_etfs", label: "Stocks + ETFs" }, { value: "stocks_crypto", label: "Stocks + Crypto" },
      ] },
      { name: "positionCount", kind: "number-chips", label: "Positions", default: 8, presets: [
        { value: 5, label: "5" }, { value: 8, label: "8" }, { value: 12, label: "12" }, { value: 20, label: "20" },
      ] },
      { name: "maxSinglePositionPct", kind: "percent", label: "Max single position", default: 0.15, helper: "Concentration cap, e.g. 15%." },
      { name: "excludeSectors", kind: "multi-chips", label: "Exclude sectors", advanced: true, options: [
      ...
      { name: "incomeVsGrowth", kind: "segmented", label: "Tilt", advanced: true, options: [
        { value: "income", label: "Income" }, { value: "balanced", label: "Balanced" }, { value: "growth", label: "Growth" },
      ] },
    ],
    buildPrompt: (v) => {
      const constraints = [
        ...
        v.maxSinglePositionPct != null && `max ${(v.maxSinglePositionPct * 100).toFixed(0)}% per position`,
  ...
  options_screener: {
    ...
    description: "Pick contracts from a single ticker's chain that match your direction, DTE, and liquidity preferences.",
    fields: [
      { name: "symbol", kind: "symbol", label: "Underlying", required: true, default: "NVDA" },
      { name: "direction", kind: "segmented", label: "Direction", required: true, default: "bullish", options: [
        { value: "bullish", label: "Bullish (calls)" },
        { value: "bearish", label: "Bearish (puts)" },
      ] },
      { name: "dteTarget", kind: "segmented", label: "Days to expiry", required: true, default: "30-45", options: [
        { value: "0-7", label: "0–7" },
      ...
      { name: "moneynessPreference", kind: "segmented", label: "Moneyness", required: true, default: "atm", options: [
        { value: "itm", label: "ITM" },
        { value: "atm", label: "ATM" },
        { value: "otm", label: "OTM" },
      ] },
      { name: "liquidityMinimum", kind: "segmented", label: "Liquidity floor", required: true, default: "tight_spreads", options: [
        { value: "any", label: "Any" },
        { value: "tight_spreads", label: "Tight spreads" },
        { value: "high_oi", label: "High open interest" },
      ] },
      { name: "objective", kind: "select", label: "Objective", required: true, default: "directional_leverage", options: [
      ...
      { name: "ivPreference", kind: "segmented", label: "IV preference", advanced: true, options: [
        { value: "any", label: "Any" }, { value: "low", label: "Low IV" }, { value: "high", label: "High IV" },
      ] },
    ],
    buildPrompt: (v) => {
      const tail = [
  ...
  compare_assets: {
    ...
    description: "Side-by-side comparison of two or more tickers across price, technicals, fundamentals, and risk.",
    fields: [
      { name: "symbols", kind: "symbols", label: "Tickers", required: true, min: 2, max: 6, default: ["NVDA", "AMD"], helper: "2 to 6 symbols." },
      { name: "metrics", kind: "multi-chips", label: "Focus on", advanced: true, options: [
