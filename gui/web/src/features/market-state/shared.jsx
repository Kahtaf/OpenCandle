...
export function ConfirmButton({ label, confirmLabel = "Confirm?", disabled, onConfirm, variant = "ghost", size = "xs", className }) {
    ...
    <section className="rounded-xl border border-border bg-card shadow-subtle-xs">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          {count !== undefined ? (
            <span className="rounded-full bg-secondary px-2 py-0.5 text-[11px] tabular-nums text-muted-foreground">{count}</span>
          ) : null}
...
export function PanelSearch({ label, filter, setFilter }) {
  return (
    <label className="relative">
      ...
      <Input className="h-8 w-44 pl-8 text-xs" placeholder={label} value={filter} onChange={(event) => setFilter(event.target.value)} />
    </label>
  );
}
...
export function SignedPercent({ value, decimals = 2 }) {
  ...
  return (
    <span className={cn("tabular-nums font-medium", value > 0 ? "text-success" : value < 0 ? "text-destructive" : "text-muted-foreground")}>
...
export function SignedMoney({ value, percent, currency = "USD" }) {
  ...
  return (
    <span className={cn("tabular-nums font-medium", value > 0 ? "text-success" : value < 0 ? "text-destructive" : "text-muted-foreground")}>
      {sign}{money(Math.abs(value), currency)}
      {typeof percent === "number" && Number.isFinite(percent) ? ` (${sign}${Math.abs(percent).toFixed(1)}%)` : ""}
    </span>
  );
}
