...
import { DeltaChip, MoneyTile, PlainOutput, RangeBar, Sparkline, StatRow, ToolCard, extractDetails, formatDateShort } from "./_shared.jsx";
...
export function MacroSeriesCard({ message, header, text }) {
  ...
  const numeric = observations
    .map((o) => ({ date: o.date, value: typeof o.value === "string" ? parseFloat(o.value) : o.value }))
  ...
  return (
    <ToolCard>
      {header}
      <div>
        <div className="text-base font-medium tracking-tight text-foreground">{d.title || d.id || "Series"}</div>
      ...
      <div className="flex flex-wrap items-baseline gap-3">
        <span className="text-3xl font-semibold tabular-nums tracking-tight text-foreground">{latest.value.toFixed(2)}</span>
      ...
      {d.lastUpdated ? (
        <div className="text-[11px] text-muted-foreground">FRED last updated {formatDateShort(d.lastUpdated)}</div>
      ) : null}
    </ToolCard>
  );
}
...
export function FearGreedCard({ message, header, text }) {
  ...
  return (
    <ToolCard>
      {header}
      <div className="flex flex-wrap items-baseline gap-3">
        <span className="text-[3rem] font-semibold leading-none tabular-nums tracking-tight text-foreground">{d.value}</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        <MoneyTile label="Prev close" value={Number.isFinite(d.previousClose) ? d.previousClose : "—"} />
