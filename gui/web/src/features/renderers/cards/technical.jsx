...
export function TechnicalIndicatorsCard({ message, header, text }) {
  ...
  if (!series) return <ToolCard>{header}<PlainOutput text={text} /></ToolCard>;
  return (
    <ToolCard>
      {header}
...
function projectSeries(d) {
  ...
  const rsi = arr(d.rsi);
  const macd = arr(d.macd).map((m) => m && typeof m === "object" ? m : null).filter(Boolean);
  const bb = arr(d.bb).map((b) => b && typeof b === "object" ? b : null).filter(Boolean);
...
function PriceChart({ series }) {
  const numeric = [...series.price, ...series.sma20, ...series.sma50, ...series.bbUpper, ...series.bbLower].filter(Number.isFinite);
  ...
  return (
    <ChartFrame label="Price" height={height} legend={[
      ...
      <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" className="block h-40 w-full">
        {envelope ? <path d={envelope} fill="hsl(var(--tw-foreground) / 0.06)" stroke="none" /> : null}
        {sma50Path ? <path d={sma50Path} fill="none" stroke="hsl(var(--tw-warning))" strokeWidth="1" strokeDasharray="3 3" vectorEffect="non-scaling-stroke" /> : null}
        {sma20Path ? <path d={sma20Path} fill="none" stroke="hsl(var(--tw-info))" strokeWidth="1" vectorEffect="non-scaling-stroke" /> : null}
        {pricePath ? <path d={pricePath} fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" vectorEffect="non-scaling-stroke" /> : null}
      </svg>
    </ChartFrame>
  );
}
...
function RsiChart({ series }) {
  ...
  return (
    <ChartFrame label="RSI(14)" height={height} valueLabel={
      ...
      <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" className="block h-14 w-full">
        <line x1="0" y1={toY(70)} x2={width} y2={toY(70)} stroke="hsl(var(--tw-warning) / 0.5)" strokeDasharray="2 2" strokeWidth="1" vectorEffect="non-scaling-stroke" />
        <line x1="0" y1={toY(30)} x2={width} y2={toY(30)} stroke="hsl(var(--tw-info) / 0.5)" strokeDasharray="2 2" strokeWidth="1" vectorEffect="non-scaling-stroke" />
        {path ? <path d={path} fill="none" stroke="currentColor" strokeWidth="1.4" vectorEffect="non-scaling-stroke" /> : null}
      </svg>
    </ChartFrame>
  );
}
...
function MacdChart({ series }) {
  ...
  const all = [...series.macdLine, ...series.macdSignal, ...series.macdHist].filter(Number.isFinite);
  ...
  return (
    <ChartFrame label="MACD" height={height} legend={[
      ...
      Number.isFinite(last) && Number.isFinite(lastSignal)
        ? <span className="tabular-nums text-muted-foreground">{last.toFixed(2)} / {lastSignal.toFixed(2)}</span>
        : null
    }>
      <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" className="block h-[70px] w-full">
        <line x1="0" y1={toY(0)} x2={width} y2={toY(0)} stroke="hsl(var(--tw-border))" strokeWidth="1" vectorEffect="non-scaling-stroke" />
        {series.macdHist.map((value, index) => {
          ...
          return (
            <line
              key={index}
              x1={x.toFixed(1)} y1={y0.toFixed(1)}
              x2={x.toFixed(1)} y2={y1.toFixed(1)}
              stroke={positive ? "hsl(var(--tw-success) / 0.6)" : "hsl(var(--tw-destructive) / 0.6)"}
              strokeWidth="1.4"
        ...
        {linePath(series.macdSignal, width, toY) ? (
          <path d={linePath(series.macdSignal, width, toY)} fill="none" stroke="hsl(var(--tw-warning))" strokeWidth="1.2" strokeDasharray="3 3" vectorEffect="non-scaling-stroke" />
        ) : null}
        {linePath(series.macdLine, width, toY) ? (
          <path d={linePath(series.macdLine, width, toY)} fill="none" stroke="currentColor" strokeWidth="1.4" vectorEffect="non-scaling-stroke" />
        ) : null}
      </svg>
    </ChartFrame>
  );
  ...
  return (
    <div className="grid gap-1.5 border-t border-border pt-2">
      <div className="flex items-baseline justify-between gap-2">
            ...
            <div className="hidden items-center gap-2 sm:flex">
              {legend.map((entry) => (
                <span key={entry.label} className="inline-flex items-center gap-1 text-[10px] text-muted-foreground">
                  <span aria-hidden="true" className="inline-block h-1 w-3 rounded-sm" style={{ background: entry.color }} />
                  {entry.label}
                </span>
              ))}
            </div>
        ...
        {valueLabel ? <span className="text-[11px] tabular-nums text-muted-foreground">{valueLabel}</span> : null}
      </div>
      {children}
    </div>
  );
...
function bandPath(upper, lower, width, toY) {
  ...
  const top = upper.map((v, i) => `${i === 0 ? "M" : "L"}${((i / denom) * width).toFixed(1)},${toY(v).toFixed(1)}`).join(" ");
  const bottom = lower.map((v, i) => `L${((i / denom) * width).toFixed(1)},${toY(v).toFixed(1)}`).reverse().join(" ");
  return `${top} ${bottom} Z`;
}
