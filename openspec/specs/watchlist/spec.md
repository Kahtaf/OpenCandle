...
## Purpose
Define how watchlist price checks fetch quotes: a single batched provider request per check with a resilient per-symbol fallback path when the batch call cannot resolve every symbol.
## Requirements
