...
- **Add a keyless TradingView scanner provider** — new `src/providers/tradingview.ts` with `screenStocks(opts)` (filtered/sorted multi-symbol scan) and `getQuotes(symbols)` (batch snapshot via explicit ticker list). Both POST to `scanner.tradingview.com/{market}/scan2`, decode the column-compressed `{fields, symbols:[{s,f}]}` response **by reading positions from the response `fields[]` (never hard-coded indices)**, tolerate missing columns as `null`, and route through the existing `cache` + `rateLimiter` + stale-fallback infra. No API key, no login, no browser, no cookies.
...
- **Add batch-quote fallback to the `watchlist` tool** — the `check` action prefers a single TradingView `getQuotes` batch call and falls back to the current per-symbol Yahoo path via `withFallback`, preserving existing behavior when TradingView is unavailable.
...
- `watchlist`: `check` action uses a single TradingView batch quote with Yahoo per-symbol fallback, eliminating the fan-out that risks Yahoo rate-limit bans.

## Impact

- **Dependencies**: None. The scanner is a plain JSON POST via existing `httpGet` — no SDK, no `better-sqlite3`/browser path (unlike our Twitter provider).
...
- **Tests**: New unit tests + fixtures for the provider, decoder, symbol mapping, and `screen_stocks` tool; updated `watchlist` tests for the batch+fallback path. A **fixture shape-guard test** asserts the decoder survives field reordering and missing columns. No live API calls in unit tests.
...
- **Endpoint shape**: `POST https://scanner.tradingview.com/{market}/scan2?label-product=screener-stock` with body `{ markets, symbols:{tickers|query}, columns, filter, filter2 (boolean tree), sort, range:[offset, offset+limit] }`. Response is column-compressed `{ totalCount, fields:[...], symbols:[{ s, f:[...] }], time }`; row limit clamps to ~500. Quotes can use `global/scan2` with an explicit ticker list.
...
- **`himself65/finance-skills`** — `opencli-plugins/tradingview/lib/scanner.js` + `symbols.js` (MIT, © Alex Yang). Borrow: the exact `scan2` request body grammar (`buildScreenerBody`, `index_filters`, `filter2` boolean composition), the **position-based `decodeScannerRows`** ("read `fields[]`, never hard-code indices") and `buildTvSymbol(exchange, ticker)`. Closest prior art to what we're building.
