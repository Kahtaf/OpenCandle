...
- [x] 1.1 Read `node_modules/@mariozechner/pi-coding-agent/dist/core/extensions/types.d.ts` around lines 551–750, focused on `ToolResultEvent`, `ToolResultEventResult`, `ExtensionAPI.on("tool_result", ...)`, and `ExtensionContext.sendMessage`. Read the runtime behavior in `node_modules/@mariozechner/pi-coding-agent/dist/core/extensions/runner.js` around line 427 where handlers are invoked. Document whether `ToolResultEventResult` supports (a) replacing `content` in place, (b) requesting a re-dispatch of the tool, (c) neither. This determines the "connect now" success-path UX copy.
