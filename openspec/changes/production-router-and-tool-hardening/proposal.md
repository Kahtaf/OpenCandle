...
3. **LLM router promotion needs proof and safety nets.** The current branch already defaults `OPENCANDLE_ROUTER_MODE` to `llm`, while the `rules` path remains available as rollback. That default should not be treated as production-ready until router evals, acronym disambiguation, and provider-layer invalid-symbol guards are green together.
...
- **Validate and keep the LLM router default only if gated.** `OPENCANDLE_ROUTER_MODE=llm` is the current branch default. Rules path stays available behind explicit `OPENCANDLE_ROUTER_MODE=rules` for one release as rollback insurance, then `remove-rule-router` (separate change) deletes it. Keeping the default is gated on a numeric acceptance bar: live eval pass-rate ≥ 90%, p95 router latency ≤ 1500ms, and cost-per-turn within budget. The current published baseline is unknown — the first task is to establish it (a prior local run produced 1/18 only because the harness shell had no `ANTHROPIC_API_KEY`, which is not a router-quality signal). If the gate fails, this change must revert the default to `rules` before merge or explicitly split the default promotion into a later change.
- **Acronym-aware entity post-filter, shared by both router paths.** Add `src/routing/symbol-disambiguator.ts` with a finance-acronym dictionary (IV, HV, ITM, OTM, ATM, IPO, SEC, FED, FOMC, IRS, ECB, BOE, BOJ, GDP, CPI, PPI, FX, MA, NDA, plus all current COMMON_WORDS finance entries). Tokens in this dictionary require a direct positive ticker signal — `$`-prefix, a local phrase such as "IV ticker", "ticker IV", "IV stock", or "symbol IV", or another explicit per-token ticker marker — before being retained as a symbol. Bare comma-list adjacency is not enough because it fails the observed `IV, ASTS` case. Wire as a post-filter that runs against `extractedEntities.symbols` regardless of which router path produced them, so the LLM router benefits from the same guardrail when it hallucinates a ticker.
...
- **Flags:** `OPENCANDLE_ROUTER_MODE` is currently `llm` by default on this branch. Rules path stays opt-in for one release if the acceptance gate is green.
