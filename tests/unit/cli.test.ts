...
describe("opencandle package commands", () => {
  ...
  it(
    "prints doctor health for doctor without starting the TUI",
    async () => {
    ...
    // import, and it measurably exceeds vitest's default 5s test timeout on
    // a loaded machine even though it stays under a second when idle. A
    // beforeAll warm-up does not help: `runCli` calls `vi.resetModules()`,
    // so the cost is re-paid on every fresh import, not just the first one.
    20_000,
  );
