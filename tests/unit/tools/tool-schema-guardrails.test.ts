...
describe("tool schema guardrails", () => {
  ...
  it("rejects free-form history periods before provider calls", () => {
    expect(Value.Check(correlationTool.parameters, { symbols: ["AAPL", "MSFT"], period: "1w" })).toBe(false);
    expect(Value.Check(correlationTool.parameters, { symbols: ["AAPL", "MSFT"], period: "2y" })).toBe(true);
    ...
    expect(Value.Check(backtestTool.parameters, { symbol: "AAPL", strategy: "sma_crossover", period: "6mo" })).toBe(false);
    expect(Value.Check(backtestTool.parameters, { symbol: "AAPL", strategy: "sma_crossover", period: "5y" })).toBe(true);
  });
