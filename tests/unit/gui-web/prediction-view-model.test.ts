...
describe("predictionProgress", () => {
  it("measures bullish progress from entry toward target", () => {
    expect(predictionProgress({ direction: "bullish", entryPrice: 100, targetPrice: 120, currentPrice: 110 }))
      .toEqual({ percent: 50, targetHit: false, directionCorrect: true });
  });

  it("clamps to 100 and flags target hit", () => {
    expect(predictionProgress({ direction: "bullish", entryPrice: 100, targetPrice: 120, currentPrice: 130 }))
      .toEqual({ percent: 100, targetHit: true, directionCorrect: true });
  });

  it("measures bearish progress downward and flags wrong-way moves", () => {
    expect(predictionProgress({ direction: "bearish", entryPrice: 100, targetPrice: 80, currentPrice: 90 }))
      .toEqual({ percent: 50, targetHit: false, directionCorrect: true });
    expect(predictionProgress({ direction: "bearish", entryPrice: 100, targetPrice: 80, currentPrice: 105 }))
      .toEqual({ percent: 0, targetHit: false, directionCorrect: false });
  });

  it("returns null when target or current price is missing", () => {
    expect(predictionProgress({ direction: "bullish", entryPrice: 100, targetPrice: null, currentPrice: 110 })).toBeNull();
    expect(predictionProgress({ direction: "bullish", entryPrice: 100, targetPrice: 120, currentPrice: null })).toBeNull();
  });
});
...
describe("buildPredictionScorecard", () => {
  it("summarizes open count, resolved hit rate, and average winning move", () => {
    const card = buildPredictionScorecard([
      ...
      { status: "resolved", resultJson: JSON.stringify({ currentPrice: 120, pnlPercent: 20, correct: true }) },
      { status: "resolved", resultJson: JSON.stringify({ currentPrice: 95, pnlPercent: 10, correct: true }) },
      { status: "expired", resultJson: JSON.stringify({ currentPrice: 90, pnlPercent: -10, correct: false }) },
      { status: "cancelled" },
    ]);
