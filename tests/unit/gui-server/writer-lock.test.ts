...
describe("writer lock", () => {
  it("acquires and releases a lock atomically", async () => {
    ...
    try {
      const acquired = await acquireWriterLock(dir, "gui", { pid: process.pid, staleGraceMs: 1000 });
      expect(acquired.role).toBe("writer");
      expect(readWriterLock(dir)?.processKind).toBe("gui");

      const follower = await acquireWriterLock(dir, "tui", { pid: process.pid, staleGraceMs: 1000 });
