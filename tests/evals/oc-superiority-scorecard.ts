...
function classifyProductReplay(report: ProductReplayComparison | undefined): ScorecardLayer {
  ...
  if (report.status === "unsupported") {
    return { status: "incomplete", reasons: [report.unsupportedReason ?? "product replay unsupported"], reportPaths: paths };
  }
  const reasons: string[] = [];
  if ((report.aggregateDelta ?? 0) < 0) reasons.push(`product replay aggregate regressed by ${report.aggregateDelta}`);
  if ((report.passDelta ?? 0) < 0) reasons.push(`product replay pass count regressed by ${report.passDelta}`);
  const regressedCases = report.caseChanges.filter((entry) => entry.status === "regressed");
  if (regressedCases.length > 0) reasons.push(`product replay regressed cases: ${regressedCases.map((entry) => entry.id).join(", ")}`);
  return { status: reasons.length > 0 ? "fail" : "pass", reasons, reportPaths: paths };
}

function classifyCompetitiveReplay(report: CompetitiveReplayComparison | undefined): ScorecardLayer {
  if (!report) return incomplete("competitive replay report missing");
  const paths = reportPathsFromRunSummary(report.current, report.base);
  if (report.status === "unsupported") {
    return { status: "incomplete", reasons: [report.unsupportedReason ?? "competitive replay unsupported"], reportPaths: paths };
  }
  const reasons: string[] = [];
  if ((report.openCandleWinDelta ?? 0) < 0) reasons.push(`competitive OpenCandle wins regressed by ${report.openCandleWinDelta}`);
  if ((report.lossDelta ?? 0) > 0) reasons.push(`competitive losses increased by ${report.lossDelta}`);
  const regressedCases = report.caseChanges.filter((entry) => entry.status === "regressed");
  if (regressedCases.length > 0) reasons.push(`competitive regressed prompts: ${regressedCases.map((entry) => entry.id).join(", ")}`);
  return { status: reasons.length > 0 ? "fail" : "pass", reasons, reportPaths: paths };
}
...
function classifyPromptPolicy(report: PromptPolicyManifestLike | undefined): ScorecardLayer {
  ...
  const reason = failed > 0 ? [`prompt-policy manifest has ${failed} failed case${failed === 1 ? "" : "s"}`] : [];
  return {
...
function classifyArchitectureSignals(report: PromptPolicyManifestLike | undefined): ScorecardLayer {
  ...
  const hasPlanning = observed.some((entry) => typeof entry.taskFamily === "string" || typeof entry.policyCardId === "string");
  const hasArtifactContracts = observed.some((entry) => Array.isArray(entry.artifactContractIds) && entry.artifactContractIds.length > 0);
  const hasEvidenceTypes = observed.some((entry) => Array.isArray(entry.evidenceTypes) && entry.evidenceTypes.length > 0);
...
function blockersFromLayers(
  layers: Record<ScorecardLayerId, ScorecardLayer>,
): ScorecardBlocker[] {
  return (Object.entries(layers) as Array<[ScorecardLayerId, ScorecardLayer]>).flatMap(([layer, result]) =>
    result.status === "fail"
      ? result.reasons.map((reason) => ({ layer, reason }))
      : []
  );
}
...
function productImprovements(report: ProductReplayComparison | undefined): string[] {
  ...
  if ((report.aggregateDelta ?? 0) > 0) improvements.push(`product replay aggregate improved by ${report.aggregateDelta}`);
  if ((report.passDelta ?? 0) > 0) improvements.push(`product replay pass count improved by ${report.passDelta}`);
  return improvements;
}
...
function competitiveImprovements(report: CompetitiveReplayComparison | undefined): string[] {
  if (!report || report.status !== "compared") return [];
  const improvements: string[] = [];
  if ((report.openCandleWinDelta ?? 0) > 0) improvements.push(`competitive replay OpenCandle wins improved by ${report.openCandleWinDelta}`);
  if ((report.lossDelta ?? 0) < 0) improvements.push(`competitive replay losses improved by ${Math.abs(report.lossDelta ?? 0)}`);
  return improvements;
}
