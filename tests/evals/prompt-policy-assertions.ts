...
export function evaluateFinalAnswerAssertion(
  ...
  const checks: Array<{ pattern: RegExp; passed: boolean; reason: string; deterministic?: boolean }> = [
    {
    ...
    },
    {
      ...
      passed: true,
      reason: "conditional ambiguity disclosure assertion; keep as qualitative judge guidance unless lookup availability is typed",
      deterministic: false,
    },
    {
    ...
    },
    {
      ...
      passed: /could not|unavailable|not verified|not verify|missing|unknown|unable|no current|provider gap/i.test(text) ||
        !/\b(?:eps|revenue|guidance|beat|miss|reported|consensus|actual)\b.{0,40}\b\d+(?:\.\d+)?\b/i.test(trace.text),
...
function evaluateManifestAssertion(
  ...
  if (lowerAssertion.includes("does not punt because a dedicated live brokerage tool is missing")) {
    return forbids(/\b(?:cannot|can't|unable)\s+(?:compare|answer|help|provide)\b/, /\b(?:no dedicated|missing).{0,40}(?:tool|provider).{0,40}(?:cannot|can't|unable)\b/);
  }
  if (lowerAssertion.includes("compares fees, expense ratios")) {
    return requiredTerms(/fees?/, /expense ratios?/, /cash sweep/, /fractional shares?/, /fund minimums?/, /tax[- ]loss/, /transfer|account fees?/, /support/, /recurring/);
  }
  if (lowerAssertion.includes("labels provider-site facts") || lowerAssertion.includes("labels current yield facts")) {
    return requiredTerms(/verify|provider site|current .* facts?|not live|cannot verify/);
  }
  if (lowerAssertion.includes("practical default") || lowerAssertion.includes("default hierarchy")) {
  ...
  if (lowerAssertion.includes("compares requested etfs before portfolio construction")) {
    return { passed: workflow !== "portfolio_builder", reason: "expected comparison mode rather than portfolio construction", deterministic: true };
  }
  ...
  if (lowerAssertion.includes("sleep test, dca")) {
    return requiredTerms(/sleep test/, /DCA|dollar[- ]cost/, /rebalance/, /position cap/, /tax/, /custody|exchange/, /emergency fund|high[- ]interest debt/);
  }
  ...
  if (lowerAssertion.includes("clear call with key risks")) {
    return requiredTerms(/buy|sell|hold|avoid|wait|trim|add|prefer|recommend/, /risk|downside/, /position|entry|size/, /confidence/, /invalidat/);
  }
  if (lowerAssertion.includes("direction and strength of sentiment")) {
    return requiredTerms(/sentiment/, /bullish|bearish|positive|negative|neutral|lean/, /strong|weak|moderate|score/);
  }
  ...
  if (lowerAssertion.includes("missing sources and why they matter")) {
    return requiredTerms(/missing|unavailable|not covered/, /source/, /matter|because|limits|confidence/);
  }
  if (lowerAssertion.includes("source-coverage risk and low sample counts")) {
    return requiredTerms(/source[- ]coverage|coverage risk/, /low sample|sample count|sample size|thin sample/);
  }
  ...
  if (lowerAssertion.includes("sec filing evidence")) {
    return { passed: tools.includes("get_sec_filings") || /sec|filing|10-k|10-q|8-k/.test(text), reason: "expected SEC filing evidence", deterministic: true };
  }
  if (lowerAssertion.includes("separates filing metadata")) {
    return requiredTerms(/filing metadata|filing/, /section|body|summary/, /news|market data|adjacent/);
  }
  ...
  if (lowerAssertion.includes("thesis-changing deltas")) {
    return requiredTerms(/thesis|change|delta/, /date|timing/, /source/, /6[- ]?12|six|twelve|month/);
  }
  if (lowerAssertion.includes("does not route as portfolio construction")) {
    return { passed: workflow !== "portfolio_builder", reason: "expected non-construction route", deterministic: true };
  }
  if (lowerAssertion.includes("does not ask for a portfolio budget")) {
    return { passed: !/need .*budget|what .*budget|provide .*budget/.test(text), reason: "expected answer not to request a portfolio budget", deterministic: true };
  }
  ...
  if (lowerAssertion.includes("guaranteed after-tax debt return")) {
    return requiredTerms(/guaranteed|certain/, /after[- ]tax|tax/, /debt|mortgage/, /uncertain|market return/);
  }
  if (lowerAssertion.includes("liquidity, emergency fund")) {
    return requiredTerms(/liquid/, /emergency fund/, /tax/, /risk tolerance/, /time horizon/, /hybrid|split/);
  }
  ...
  if (lowerAssertion.includes("hedge floor, premium")) {
    return requiredTerms(/hedge floor|floor/, /premium/, /delta|theta|greeks?/, /liquidity/, /risk/);
  }
  if (lowerAssertion.includes("bottom-line portfolio risk/reward") || lowerAssertion.includes("bottom-line structural portfolio read")) {
    return requiredTerms(/bottom line/, /portfolio/, /risk|reward|structural/);
  }
  if (lowerAssertion.includes("current macro evidence")) {
    return requiredTerms(/current macro evidence|macro evidence/, /structural portfolio read|structural/, /sleeve/, /risk|opportunit/, /actionable|adjust/, /watchlist|invalidat/);
  }
  if (lowerAssertion.includes("unavailable macro data")) {
    return requiredTerms(/unavailable|missing|data gap|not available/, /macro|inflation|rates?|fed|sentiment/);
  }
  if (lowerAssertion.includes("diversification, concentration, geography")) {
    return requiredTerms(/diversif/, /concentration/, /geograph|international|regional/, /duration/, /credit/, /liquid/, /tax/, /horizon/, /rebalance/);
  }
  ...
  if (lowerAssertion.includes("specific current facts that would improve")) {
    return requiredTerms(/would improve|need|specific current facts|verify/, /inflation|rates?|sentiment|macro|data/);
  }
  ...
  if (lowerAssertion.includes("strategy return, buy-and-hold")) {
    return requiredTerms(/strategy return/, /buy[- ]and[- ]hold/, /outperformance|underperformance/, /trade count|trades/, /win rate/, /max drawdown/);
  }
  ...
  if (lowerAssertion.includes("track_prediction")) {
    return { passed: tools.includes("track_prediction"), reason: "expected track_prediction tool call", deterministic: true };
  }
