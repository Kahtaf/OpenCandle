...
export const ANALYSIS_RUBRIC = [
  {
    ...
    criterion:
      "The response references data from multiple tool categories (e.g., price data + fundamentals + technicals, not just one source).",
    examples: {
  ...
  },
  {
    ...
    criterion:
      "The response includes explicit quantitative screening criteria with PASS/FAIL or clear threshold-based assessments.",
    examples: {
  ...
  },
  {
    ...
    criterion:
      "The response explicitly mentions risk metrics such as volatility, max drawdown, VaR, beta, or specific downside scenarios.",
    examples: {
  ...
  },
  {
    ...
    criterion:
      "The response follows a 'Because [data] + [data], I conclude [thesis]' pattern, explicitly connecting data points to conclusions.",
    examples: {
  ...
  },
  {
    ...
    criterion:
      "The response provides a clear directional view (buy/hold/sell or equivalent) with a conviction level (high/medium/low).",
    examples: {
...
export const DEBATE_RUBRIC = [
  {
    ...
    criterion:
      "The synthesis explicitly acknowledges competing viewpoints and explains why one outweighs the other, rather than averaging or ignoring disagreement.",
    examples: {
  ...
  },
  {
    ...
    criterion:
      "The conclusion states a specific, testable condition (metric + threshold + timeframe) that would reverse the verdict.",
    examples: {
  ...
  },
  {
    ...
    criterion:
      "The analysis concedes genuine weaknesses in its own position — naming specific data points that support the opposing view — rather than dismissing all counterarguments.",
    examples: {
...
export function buildJudgePrompt(
  rubricItem: (typeof ANALYSIS_RUBRIC)[number],
  responseText: string,
): string {
